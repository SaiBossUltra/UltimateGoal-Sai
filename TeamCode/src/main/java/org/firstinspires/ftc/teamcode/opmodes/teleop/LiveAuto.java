...
@Autonomous(name="Main Auto UDontCommitAnything", group="autonomous")
//@Disabled
public class LiveAuto extends LiveTeleopBase {
    ...
    public void on_start() {
        ...
        if(pattren == 2){
            robot.drive_train.odo_move(robot.drive_train.get_X(), robot.drive_train.get_Y(), robot.drive_train.get_A(), 1, distance_acc, angle_acc);
        }else {
        ...
        if(pattren == 3) { // seperate shooting positions for 4-ring auto and the other set ups
            ...
            robot.drive_train.odo_move(-12, 0, Math.PI - ((Math.PI/180)*5), 1, distance_acc, angle_acc); // shooting position for 1-ring auto
        } else {
            robot.drive_train.odo_move(-10, 0, Math.PI, 1, distance_acc, angle_acc);
        }
        ...
        if(pattren > 1){ //make sure it is either 1 or 4 ring starting stack before trying to intake to shoot rings
            ...
            if(pattren == 2){
                //shootpos(pattren);
                robot.drive_train.odo_move(-12, -2, Math.PI - ((Math.PI/180)*8) , 1, distance_acc, angle_acc); // go to shooting position
    ...
    private void SCWobble(int pattren){
        ...
        robot.drive_train.odo_move(robot.drive_train.get_X(), robot.drive_train.get_Y(), robot.drive_train.get_A() + ((Math.PI/180)*80), 1, distance_acc, Math.PI/360);
        sleep(50);
        robot.drive_train.odo_move(robot.drive_train.get_X() + 8, robot.drive_train.get_Y(), robot.drive_train.get_A(), 1, distance_acc, angle_acc);
        sleep(500);
    ...
    private void intake(int pattren){
        ...
        if(pattren == 3){
           // robot.drive_train.odo_move(-10, 0, robot.drive_train.get_A() + ((Math.PI/180)*4.5) , 1, distance_acc, angle_acc);
            robot.drive_train.odo_move(-12, 0, Math.PI , 1, distance_acc, angle_acc);
            ...
            if(pattren == 3){
                ...
                robot.drive_train.odo_move(robot.drive_train.get_X(), -37.0, robot.drive_train.get_A(), 0.6, distance_acc, angle_acc, 0, 1);
            }
    ...
    private void gotoRectBasedOnPattren(int pattren) {
        ...
        robot.drive_train.odo_move(-25.0, -12.0, Math.PI, 1, distance_acc, angle_acc);
        switch (pattren) {
            case 2:
